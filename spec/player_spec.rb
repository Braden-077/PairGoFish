...
describe Player do
  describe '#initalize' do
    ...
    it 'initializes with given paramaters' do 
      player = Player.new(name: 'Braden', hand: [Card.new('A', 'H')], books: ['A', 'J'])
  ...
  describe '#take_cards' do
    it 'takes the card(s) passed in' do 
      player = Player.new(name: 'Braden', hand: [Card.new('A', 'H')])
  ...
  describe '#give_cards' do
    it 'gives the cards matching the rank' do 
      player = Player.new(name: 'Braden', hand: [Card.new('A', 'H'), Card.new('A', 'S'), Card.new('2', 'S')])
  ...
  describe '#check_for_books' do
    it 'checks for books and deletes the correct cards' do
      player = Player.new(name: 'Braden', hand: [Card.new('A', 'H'), Card.new('2', 'S'), Card.new('A', 'D')])
  ...
  describe '#sort_hand' do
    it 'sorts the hand' do 
      player = Player.new(name: 'Braden', hand: [Card.new('Q', 'S'), nil, nil, nil, Card.new('2', 'H'), nil, Card.new('7', 'D'), Card.new('8', 'C'), nil])
      player.sort_hand
  ...
  describe '#has_rank?' do
    it 'checks for the card in a player\'s hand' do 
      player = Player.new(name: 'Braden', hand: [Card.new('A', 'H'), Card.new('A', 'S'), Card.new('A', 'D'), Card.new('2', 'S')])
